...
void ds_write1bit(void)
{
    ...
    //Make the bus go low by making it a 1
    DDRC |= (1 << PC3);
    ...
    //Delay of 60 us to meet time cycle
    _delay_us(60);
}
...
void ds_write0bit(void)
{
    ...
    //Hold the bus for 60 us
    _delay_us(60);

    //Release the bus
    DDRC &= ~(1 << PC3);

    //Delay of 2 us to meet time cycle
    _delay_us(2);
}
...
unsigned char ds_readbit(void)
{
    ...
    //Release the bus to go high by making it a 0
    DDRC &= ~(1 << PC3);
    ...
    //Sampling the bus 
    //With a time window of 15 us
    bit = (PINC & (1 << PC3));
    
    //Delay to meet the requirement of 62 us time cycle
    _delay_us(50);
